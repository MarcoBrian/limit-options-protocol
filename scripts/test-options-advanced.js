...
async function main() {
  ...
  // Deploy Options NFT (the original one, not SimpleOptionsNFT)
  ...
  try {
    ...
    // The OptionsNFT expects: (address maker, uint256 nonce, uint256 expiry, uint8 v, bytes32 r, bytes32 s)
    console.log(`\n🧪 Creating interaction data for OptionsNFT...`);
    
    const interactionData = ethers.AbiCoder.defaultAbiCoder().encode(
      ["address", "uint256", "uint256", "uint8", "bytes32", "bytes32"],
      [maker.address, nonce, expiry, v2, r2, s2]
    );
    ...
    try {
        ...
        console.log(`- Decoded nonce: ${decodedParams[1]}`);
        console.log(`- Decoded expiry: ${decodedParams[2]}`);
        console.log(`- Decoded v: ${decodedParams[3]}`);
        console.log(`- Decoded r: ${decodedParams[4]}`);
        console.log(`- Decoded s: ${decodedParams[5]}`);
    } catch (error) {
        ...
        try {
            ...
            console.log(`  - nonce: ${originalDecoded[1]}`);
            console.log(`  - expiry: ${originalDecoded[2]}`);
            console.log(`  - v: ${originalDecoded[3]}`);
            console.log(`  - r: ${originalDecoded[4]}`);
            console.log(`  - s: ${originalDecoded[5]}`);
        } catch (error2) {
